package chap02_multi_dimension;

public class MultiDimensionEx {

  public static void main(String[] args) {
    
    //2차원 배열이란?
    //1차원 배열이 여러 개 존재하는 경우 이 1차원 배열을 합치는 것을 의미합니다.
    
    //2차원 배열 선언
    String[][] seats;  
    
    //2차원 배열 생성
    seats = new String[3][];    //1차원 배열이 3개입니다.
    
    //1차원 배열 생성
    seats[0] = new String[2];    //첫 번째 1차원 배열의 길이는 2입니다. seats[0]가 1차원 배열의 이름입니다.
    seats[1] = new String[5];    //두 번째 1차원 배열의 길이는 5입니다. seats[1]가 1차원 배열의 이름입니다.
    seats[2] = new String[4];    //세 번째 1차원 배열의 길이는 4입니다. seats[2]가 1차원 배열의 이름입니다.
    
    //2차원 배열 요소 사용(인덱스를 2개 사용: 1. 몇 번째 1차원 배열인가?  2. 1차원 배열 내 몇 번째 요소인가?)
    seats[0][0] = "신성수";
    seats[0][1] = "이동욱";
    
    seats[1][0] = "이유천";
    seats[1][1] = "조기완";
    seats[1][2] = "최미영";
    seats[1][3] = "박진성";
    seats[1][4] = "김민석";
    
    seats[2][0] = "유비";
    seats[2][1] = "이창민";
    seats[2][2] = "권형택";
    seats[2][3] = "김경진";
    
    //2차원 배열 순회 1(일반 for문)
    for(int i = 0; i < seats.length; i++) {    //각 1차원 배열 찾아가기
      for(int j = 0; j < seats[i].length; j++) {    //찾아간 1차원 배열 요소 순회하기
        System.out.print(seats[i][j] + ",");
      }
      System.out.println();
    }
    
    //2차원 배열 순회 2(향상된 for문)
    for(String[] lines: seats) {    //2차원 배열이 가지고 있는 건 1차원 배열입니다.
      for(String seat : lines) {
        System.out.print(seat + ",");
      }
      System.out.println();
    }
    
    /*
     * [Stack] -> 지역 변수(기본형 변수, 참조형 변수)가 저장되는 곳
     * |
     * |                     Reference(참조값)
     * +-------------------+
     * | seats    : 0x100  |
     * +-------------------+
     *
     * 
     * [Heap] -> new를 이용해 생성한 객체나 배열이 저장되는 곳 
     * |               
     * +-------------------+
     * | seats[0] : 0xAAA  | 0x100
     * +-------------------+
     * | seats[1] : 0xBBB  | 0x104(참조 변수 크기를 4바이트로 가정한 상황)
     * +-------------------+
     * | seats[2] : 0xCCC  | 0x108
     * +-------------------+
     * | ...               |
     * +-------------------+
     * |                   |                      [String Constant Pool] -> 문자열 리터럴("")로 생성한 문자열이 저장되는 곳
     * +-------------------+                      +--------+
     * | seats[0][0] : 0x1 | 0xAAA                | 신성수 | 0x1
     * +-------------------+                      +--------+
     * | seats[0][1] : 0x2 |                      | 이동욱 | 0x2
     * +-------------------+                      +--------+
     * | ...               |
     * +-------------------+                      +--------+
     * | seats[1][0] : 0x3 | 0xBBB                | 이유천 | 0x3
     * +-------------------+                      +--------+
     * | seats[1][1] : 0x4 |                      | 조기완 | 0x4
     * +-------------------+                      +--------+
     * | seats[1][2] : 0x5 |                      | 최미영 | 0x5
     * +-------------------+                      +--------+
     * | seats[1][3] : 0x6 |                      | 박진성 | 0x6
     * +-------------------+                      +--------+
     * | seats[1][4] : 0x7 |                      | 김민석 | 0x7
     * +-------------------+                      +--------+
     * | ...               |
     * +-------------------+                      +--------+
     * | seats[2][0] : 0x8 | 0xCCC                | 유  비 | 0x8
     * +-------------------+                      +--------+
     * | seats[2][1] : 0x9 |                      | 이창민 | 0x9
     * +-------------------+                      +--------+
     * | seats[2][2] : 0xA |                      | 권형택 | 0xA
     * +-------------------+                      +--------+
     * | seats[2][3] : 0xB |                      | 김경진 | 0xB
     * +-------------------+                      +--------+
     * | ...               |
     * +-------------------+
     */
    
//    function a() {
//      String[] array = new String[3];
//      b(array);
//      System.out.println(Arrays.toString(array));  //["a", "b", "c"]
//      int n = 0;
//      c(n);
//      System.out.println(n);  // 0
//      }
//
//      funtion b(String[] array) {
//      array[0] = "a";
//      array[1] = "b";
//      array[2] = "c";
//      }
//
//      function c(int n) {
//      n = 10;
//      }
    
  }

}
