package chap08_static;
  ...
  /*
   * 메모리 영역별 역할
   * 1. Stack : 메소드 호출마다 생성되는 영역. 지역변수, 매개변수 저장
   * 2. Heap  : new 명령으로 생성한 객체(인스턴스 변수), 배열 등 저장
   * 3. Method: 클래스 정보와 메소드 정보 등 메타데이터, static 변수 등 저장 
   */
   ...
   *    클래스 로더(Class Loader)가 실행에 필요한 클래스를 JVM의 Method 영역에 동적으로 로드(적재)합니다.
   * 2. 링킹(Linking)
   *    로드된 클래스에 대해 검증, 준비, 해결 단계를 통해 점검합니다.
   * 3. 초기화(Initialization) 
   *    static 변수에 명시된 값으로 초기화하고, static 블록을 실행합니다. << 객체 생성 이전에 실행하기 위해서 main 메소드는 static이 붙음
   * 4. main() 메소드 실행
   *    main 메소드와 그 안에서 호출되는 다른 모든 메소드는 JVM의 Stack 영역에 각각의 스택 프레임을 생성(= 공간을 만든다)합니다.
   * 5. 인스턴스 생성
   ...
   *    2) 객체.멤버(추천하지 않음!, 이유는 static 특징-3번)
