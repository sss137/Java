 ...
 * 1. 1970.01.01 00:00:00 UTC(Epoch Time) 기준으로 현재 시각을 밀리초 단위로 반환합니다.(long 타입)
 * 2. 로그, 만료 시간 계산 등의 작업을 수행할 수 있습니다.
 * 3. 시스템과 OS의 영향을 받습니다.
 */

//second > ms(밀리, 1초/1000) > ㎲(마이크로, 1초/1백만) > ㎱(나노, 1초/10억) > ㎰(피코, 1초/1조)

/*
 * System.nanoTime()
 * 1. 임의의 기준 시점으로 부터 경과한 나노초 단위 값을 반환합니다.(long 타입)
