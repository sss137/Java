...
//    System.out.println(Math.random() * 3);           //0.0 <= x < 3.0
//    System.out.println((int)(Math.random() * 3));      //0 <= x < 3
//    System.out.println((int)(Math.random() * 3 + 1));  //1 <= x < 4
  ...
  public static void secureRandom() {
     ...
     * 2. 시드(Seed) 기반 난수 생성기, 동일한 시드라도 동일한 결과를 보장하지 않습니다.
     * 3. 암호화 키, 토큰, 세션ID 등 보안이 필요한 값에는 SecureRandom 사용이 권장됩니다.
     */
    
    //SecureRandom 객체 생성(디폴트 알고리즘 사용)
    SecureRandom sr = new SecureRandom();
    
    //난수 만들기
    System.out.println(sr.nextDouble());    //0 <= x < 1.0
    System.out.println(sr.nextInt());       //int 범위 중 하나의 난수
    System.out.println(sr.nextInt(5) + 1);  //1 <= x < 6
  ...
  public static void uuid() {
    ...
    System.out.println(randomText.replace("-", ""));
